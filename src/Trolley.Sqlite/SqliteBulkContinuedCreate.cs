...
public class SqliteBulkContinuedCreate<TEntity> : ContinuedCreate<TEntity>, ISqliteCreated<TEntity>, ISqliteBulkContinuedCreate<TEntity>
{
    ...
    #region Output
    public ISqliteBulkCreated<TEntity, TResult> Output<TResult>(params string[] fieldNames)
    {
        this.DialectVisitor.Output(fieldNames);
        return new SqliteBulkCreated<TEntity, TResult>(this.DbContext, this.Visitor);
    }
    public ISqliteBulkCreated<TEntity, TResult> Output<TResult>(Expression<Func<TEntity, TResult>> fieldsSelector)
    {
