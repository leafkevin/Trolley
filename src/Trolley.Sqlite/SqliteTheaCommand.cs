...
class SqliteTheaCommand : ITheaCommand
{
    ...
    public SqliteTheaCommand(SQLiteCommand command) : this(command, null, null) { }
    public SqliteTheaCommand(SQLiteCommand command, ITheaConnection connection, ITheaTransaction transaction)
    {
    ...
    public ITheaDataReader ExecuteReader(CommandSqlType sqlType, CommandBehavior behavior = default)
    {
        ...
        return new SqliteTheaDataReader(reader);
    }
    public async Task<ITheaDataReader> ExecuteReaderAsync(CommandSqlType sqlType, CommandBehavior behavior = default, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var myReader = await this.command.ExecuteReaderAsync(behavior, cancellationToken);
            reader = myReader as SQLiteDataReader;
        }
        catch (Exception ex)
        {
            exception = ex;
