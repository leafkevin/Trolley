...
class RepositoryHelper
{
    ...
    private static ConcurrentDictionary<int, Action<ISqlVisitor, string, List<UpdateField>, List<IDbDataParameter>, object>> updateMultiSetFieldsCache = new();
    ...
    private static ConcurrentDictionary<int, Action<IDbCommand, IOrmProvider, string, StringBuilder, int, object>> deleteMultiBatchCommandInitializerCache = new();
    private static ConcurrentDictionary<int, Func<IDbCommand, IOrmProvider, object, string>> deleteCommandInitializerCache = new();
    private static ConcurrentDictionary<int, Func<IDbCommand, IOrmProvider, string, object, string>> deleteMultiCommandInitializerCache = new();
    ...
    public static Action<ISqlVisitor, string, List<UpdateField>, List<IDbDataParameter>, object> BuildMultiUpdateSetWithParameters(ISqlVisitor sqlVisitor, Type entityType, object updateObj, bool isExceptKey)
    {
        Action<ISqlVisitor, string, List<UpdateField>, List<IDbDataParameter>, object> setFieldsInitializer = null;
        if (updateObj is IDictionary<string, object> dict)
        {
            Action<ISqlVisitor, string, List<UpdateField>, List<IDbDataParameter>, IDictionary<string, object>> dictSetFieldsInitializer = null;
            if (isExceptKey)
            {
                dictSetFieldsInitializer = (visitor, prefix, setFields, dbParameters, dict) =>
                {
                    var entityMapper = visitor.MapProvider.GetEntityMap(entityType);
                    foreach (var item in dict)
                    {
                        ...
                        var parameterName = $"{visitor.OrmProvider.ParameterPrefix}{prefix}{item.Key}";
                        setFields.Add(new UpdateField { MemberMapper = memberMapper, Value = parameterName });
                        var dbParameter = visitor.OrmProvider.CreateParameter(memberMapper, parameterName, dict[item.Key]);
                        dbParameters.Add(dbParameter);
                    }
            ...
            setFieldsInitializer = (visitor, prefix, setFields, dbParameters, parameters)
                => dictSetFieldsInitializer.Invoke(visitor, prefix, setFields, dbParameters, dict);
        }
        else
        {
            var parameterType = updateObj.GetType();
            var cacheKey = HashCode.Combine(sqlVisitor.DbKey, sqlVisitor.OrmProvider, sqlVisitor.MapProvider, entityType, parameterType, isExceptKey);
            if (!updateMultiSetFieldsCache.TryGetValue(cacheKey, out setFieldsInitializer))
            {
                var entityMapper = sqlVisitor.MapProvider.GetEntityMap(entityType);
                var memberInfos = parameterType.GetMembers(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                    .Where(f => f.MemberType == MemberTypes.Property | f.MemberType == MemberTypes.Field).ToList();
                ...
                foreach (var memberInfo in memberInfos)
                {
                    ...
                    var memberMapperExpr = Expression.Constant(memberMapper);
                    var concatMethodInfo = typeof(string).GetMethod(nameof(string.Concat), new Type[] { typeof(string), typeof(string), typeof(string) });
                    var parameterNameExpr = Expression.Call(concatMethodInfo, Expression.Constant(sqlVisitor.OrmProvider.ParameterPrefix),
                        prefixExpr, Expression.Constant(memberMapper.MemberName));
                    ...
                    var addMethodInfo = typeof(List<IDbDataParameter>).GetMethod(nameof(List<IDbDataParameter>.Add));
                    blockBodies.Add(Expression.Call(dbParametersExpr, addMethodInfo, dbParameterExpr));

                    var newExpr = Expression.New(typeof(UpdateField).GetConstructor(Type.EmptyTypes));
                    var typePropertyInfo = typeof(UpdateField).GetProperty(nameof(UpdateField.Type));
                    var memberMapperPropertyInfo = typeof(UpdateField).GetProperty(nameof(UpdateField.MemberMapper));
                    var valuePropertyInfo = typeof(UpdateField).GetProperty(nameof(UpdateField.Value));
                    var setFiledExpr = Expression.MemberInit(newExpr, Expression.Bind(typePropertyInfo, Expression.Constant(UpdateFieldType.SetField)),
                        Expression.Bind(memberMapperPropertyInfo, memberMapperExpr), Expression.Bind(valuePropertyInfo, parameterNameExpr));                   

                    addMethodInfo = typeof(List<UpdateField>).GetMethod(nameof(List<UpdateField>.Add));
                    blockBodies.Add(Expression.Call(setFieldsExpr, addMethodInfo, setFiledExpr));
                }
                setFieldsInitializer = Expression.Lambda<Action<ISqlVisitor, string, List<UpdateField>, List<IDbDataParameter>, object>>(
                    Expression.Block(blockParameters, blockBodies), visitorExpr, prefixExpr, setFieldsExpr, dbParametersExpr, parameterExpr).Compile();
                updateMultiSetFieldsCache.TryAdd(cacheKey, setFieldsInitializer);
            }
        }
        return setFieldsInitializer;
    }
    ...
    public static Action<IDbCommand, IOrmProvider, IEntityMapProvider, string, StringBuilder, int, object> BuildMultiDeleteBatchCommandInitializer(IDbConnection connection, IOrmProvider ormProvider, IEntityMapProvider mapProvider, Type entityType, object deleteObjs, out bool isNeedEndParenthesis)
    {
        Action<IDbCommand, IOrmProvider, IEntityMapProvider, string, StringBuilder, int, object> commandInitializer = null;
        var entityMapper = mapProvider.GetEntityMap(entityType);
        if (entityMapper.KeyMembers.Count <= 0)
        ...
        if (deleteObj is Dictionary<string, object> dict)
        {
            Action<IDbCommand, IOrmProvider, IEntityMapProvider, string, StringBuilder, int, Dictionary<string, object>> dictCommandInitializer = null;
            if (entityMapper.KeyMembers.Count > 1)
            {
                dictCommandInitializer = (command, ormProvider, mapProvider, prefix, builder, index, dict) =>
                {
                    ...
                    foreach (var keyMapper in entityMapper.KeyMembers)
                    {
                        if (keyIndex > 0) builder.Append(" AND ");
                        var fieldName = ormProvider.GetFieldName(keyMapper.FieldName);
                        string parameterName = ormProvider.ParameterPrefix + prefix + keyMapper.MemberName + index.ToString();
                        builder.Append($"{fieldName}={parameterName}");
                        command.Parameters.Add(ormProvider.CreateParameter(keyMapper, parameterName, dict[keyMapper.MemberName]));
                        keyIndex++;
                    }
            ...
            commandInitializer = (command, ormProvider, mapProvider, prefix, builder, index, parameters) =>
               dictCommandInitializer.Invoke(command, ormProvider, mapProvider, prefix, builder, index, dict);
        }
        else
        {
            var parameterType = deleteObj.GetType();
            var cacheKey = HashCode.Combine(connection, ormProvider, entityType, parameterType);
            if (!deleteMultiBatchCommandInitializerCache.TryGetValue(cacheKey, out var objCommandInitializer))
            {
                var memberInfos = parameterType.GetMembers(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                    .Where(f => f.MemberType == MemberTypes.Property | f.MemberType == MemberTypes.Field).ToList();
                var commandExpr = Expression.Parameter(typeof(IDbCommand), "cmd");
                ...
                var appendMethodInfo1 = typeof(StringBuilder).GetMethod(nameof(StringBuilder.Append), new Type[] { typeof(char) });
                var appendMethodInfo2 = typeof(StringBuilder).GetMethod(nameof(StringBuilder.Append), new Type[] { typeof(string) });
                var concatMethodInfo = typeof(string).GetMethod(nameof(string.Concat), new Type[] { typeof(string), typeof(string), typeof(string), typeof(string) });
                if (entityMapper.KeyMembers.Count > 1)
                {
                    var addCommaExpr = Expression.Call(builderExpr, appendMethodInfo1, Expression.Constant(';'));
                    var greatThenExpr = Expression.GreaterThan(indexExpr, Expression.Constant(0, typeof(int)));
                    ...
                    var sql = $"DELETE FROM {ormProvider.GetTableName(entityMapper.TableName)} WHERE ";
                    blockBodies.Add(Expression.Call(builderExpr, appendMethodInfo2, Expression.Constant(sql)));
                    int index = 0;
                    foreach (var keyMapper in entityMapper.KeyMembers)
                    {
                        if (index > 0)
                        ...
                        var suffixExpr = Expression.Call(indexExpr, typeof(int).GetMethod(nameof(int.ToString), Type.EmptyTypes));
                        var parameterNameExpr = Expression.Call(concatMethodInfo, Expression.Constant(ormProvider.ParameterPrefix),
                            prefixExpr, Expression.Constant(keyMapper.MemberName), suffixExpr);
                        blockBodies.Add(Expression.Call(builderExpr, appendMethodInfo2, Expression.Constant($"{ormProvider.GetFieldName(keyMapper.FieldName)}=")));
                        blockBodies.Add(Expression.Call(builderExpr, appendMethodInfo2, parameterNameExpr));
                ...
                else
                {
                    ...
                    var addCommaExpr = Expression.Call(builderExpr, appendMethodInfo1, Expression.Constant(','));
                    var deldeteHead = $"DELETE FROM {ormProvider.GetTableName(entityMapper.TableName)} WHERE {ormProvider.GetFieldName(keyMapper.FieldName)} IN (";
                    var AddHeadExpr = Expression.Call(builderExpr, appendMethodInfo2, Expression.Constant(deldeteHead));
                    var greatThenExpr = Expression.GreaterThan(indexExpr, Expression.Constant(0, typeof(int)));
                    blockBodies.Add(Expression.IfThenElse(greatThenExpr, addCommaExpr, AddHeadExpr));
                ...
                objCommandInitializer = Expression.Lambda<Action<IDbCommand, IOrmProvider, string, StringBuilder, int, object>>(
                    Expression.Block(blockParameters, blockBodies), commandExpr, ormProviderExpr, prefixExpr, builderExpr, indexExpr, parameterExpr).Compile();
                deleteMultiBatchCommandInitializerCache.TryAdd(cacheKey, objCommandInitializer);
            }
            commandInitializer = (command, ormProvider, mapProvider, prefix, builder, index, parameters) =>
               objCommandInitializer.Invoke(command, ormProvider, prefix, builder, index, parameters);
        }
    ...
    public static Func<IDbCommand, IOrmProvider, string, object, string> BuildMultiDeleteCommandInitializer(IDbConnection connection, IOrmProvider ormProvider, IEntityMapProvider mapProvider, Type entityType, object parameters)
    {
        Func<IDbCommand, IOrmProvider, string, object, string> commandInitializer = null;
        if (parameters is Dictionary<string, object>)
        {
            commandInitializer = (command, ormProvider, prefix, parameter) =>
            {
                int index = 0;
                var dict = parameter as Dictionary<string, object>;
                var entityMapper = mapProvider.GetEntityMap(entityType);
        ...
        else
        {
            var parameterType = parameters.GetType();
            var cacheKey = HashCode.Combine(connection, ormProvider, entityType, parameterType);
            if (!deleteMultiCommandInitializerCache.TryGetValue(cacheKey, out commandInitializer))
            {
                int index = 0;
                var entityMapper = mapProvider.GetEntityMap(entityType);
                var commandExpr = Expression.Parameter(typeof(IDbCommand), "cmd");
                ...
                blockParameters.Add(builderExpr);
                var constructor = typeof(StringBuilder).GetConstructor(new Type[] { typeof(string) });
                var wherePrefixExpr = Expression.Constant($"DELETE FROM {ormProvider.GetTableName(entityMapper.TableName)} WHERE ");
                blockBodies.Add(Expression.Assign(builderExpr, Expression.New(constructor, wherePrefixExpr)));
                ...
                foreach (var keyMapper in entityMapper.KeyMembers)
                {
                    ...
                    if (index > 0)
                        blockBodies.Add(Expression.Call(builderExpr, appendMethodInfo, Expression.Constant(" AND ")));
                    var parameterNameExpr = Expression.Call(concatMethodInfo, Expression.Constant(ormProvider.ParameterPrefix), prefixExpr, Expression.Constant(keyMapper.MemberName));
                    blockBodies.Add(Expression.Call(builderExpr, appendMethodInfo, Expression.Constant($"{ormProvider.GetFieldName(keyMapper.FieldName)}=")));
                    blockBodies.Add(Expression.Call(builderExpr, appendMethodInfo, parameterNameExpr));
                ...
                commandInitializer = Expression.Lambda<Func<IDbCommand, IOrmProvider, string, object, string>>(Expression.Block(blockParameters, blockBodies), commandExpr, ormProviderExpr, prefixExpr, parameterExpr).Compile();
                deleteMultiCommandInitializerCache.TryAdd(cacheKey, commandInitializer);
            }
        }
        return commandInitializer;
    }
