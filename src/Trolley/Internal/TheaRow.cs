...
internal sealed partial class TheaRow : IDictionary<string, object>, IReadOnlyDictionary<string, object>
{
    private readonly TheaTable table;
    private object[] values;

    public TheaRow(TheaTable table, object[] values)
    {
    ...
    public bool TryGetValue(string key, out object value) => TryGetValue(table.IndexOfName(key), out value);
    ...
    {
        var builder = new StringBuilder("{TheaRow");
        foreach (var kv in this)
        {
            var value = kv.Value;
            builder.Append(", ").Append(kv.Key);
            if (value != null)
            {
                builder.Append(" = '").Append(kv.Value).Append('\'');
            }
            else
            {
                builder.Append(" = NULL");
    ...
    bool IDictionary<string, object>.Remove(string key) => Remove(table.IndexOfName(key));
