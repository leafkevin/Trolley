...
public interface IPostgreSqlCreate<TEntity> : ICreate<TEntity>
{
    ...
    /// <returns>返回插入对象</returns>
    new IPostgreSqlCreate<TEntity> UseTable(Func<TEntity, object[]> dependOnFieldValuesSelector);
    /// <summary>
    ...
    new IPostgreSqlFromCommand<TEntity, T> From<T>();
    /// <summary>
    /// 使用2个表创建子查询对象
    /// </summary>
    ...
    /// <returns>返回查询对象</returns>
    new IPostgreSqlFromCommand<TEntity, T1, T2> From<T1, T2>();
    /// <summary>
    /// 使用3个表创建子查询对象
    /// </summary>
    /// <typeparam name="T1">表T1实体类型</typeparam>
    ...
    /// <returns>返回查询对象</returns>
    new IPostgreSqlFromCommand<TEntity, T1, T2, T3> From<T1, T2, T3>();
    /// <summary>
    /// 使用4个表创建子查询对象
    /// </summary>
    /// <typeparam name="T1">表T1实体类型</typeparam>
    ...
    /// <returns>返回查询对象</returns>
    new IPostgreSqlFromCommand<TEntity, T1, T2, T3, T4> From<T1, T2, T3, T4>();
    /// <summary>
    /// 使用5个表创建子查询对象
    /// </summary>
    /// <typeparam name="T1">表T1实体类型</typeparam>
    ...
    /// <returns>返回查询对象</returns>
    new IPostgreSqlFromCommand<TEntity, T1, T2, T3, T4, T5> From<T1, T2, T3, T4, T5>();
    /// <summary>
    /// 使用6个表创建子查询对象
    /// </summary>
    /// <typeparam name="T1">表T1实体类型</typeparam>
    ...
    /// <returns>返回查询对象</returns>
    new IPostgreSqlFromCommand<TEntity, T1, T2, T3, T4, T5, T6> From<T1, T2, T3, T4, T5, T6>();
    #endregion
    ...
    /// <returns>返回查询对象</returns>
    new IPostgreSqlFromCommand<TEntity, T> FromQuery<T>(Expression<Func<IFromQuery, IQuery<T>>> subQueryExpr);
    #endregion
}
