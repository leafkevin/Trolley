...
public interface IPostgreSqlIncludableQuery<T, TMember> : IIncludableQuery<T, TMember>
{
    ...
    /// <summary>
    /// 根据TMasterSharding主表分表名与当前<typeparamref name="TMember"/>表分表名的映射关系，设置T表分表名获取委托确定分表名。第一个参数是TMasterSharding主表原始名称，
    /// 如：分表sys_user_105，按租户分表，原始表sys_user，第二个参数是当前<typeparamref name="TMember"/>表原始名称，第三个参数是TMasterSharding主表分表名称，返回值是当前<typeparamref name="TMember"/>表分表名称，如：查询104，105租户的2020年1月以后的订单信息和买家信息，订单表按照租户+时间(年月)分表，用户按照租户分表
    /// <code>
    ...
    /// <summary>
    /// 根据字段值，手动指定<typeparamref name="TMember"/>表分表名，可多次调用，字段值的顺序与配置的分表规则字段顺序保持一致，至少包含1个字段值，最多支持3个字段值，如：.UseTableBy(DateTime.Now)，.UseTableBy(1, 6, DateTime.Now)等
    /// </summary>
    ...
    //// <summary>
    /// 根据1个字段范围值，手动指定<typeparamref name="TMember"/>表分表名执行查询，通常是日期规则分表使用，如：.UseTableByRange(DateTime.Now.AddDays(-7), DateTime.Now)//时间分表，最近一周的订单
    /// </summary>
    ...
    /// <summary>
    /// 根据1个固定字段值和1个字段值范围值，手动指定<typeparamref name="TMember"/>表分表名执行查询，字段值的顺序与配置的字段顺序保持一致，通常是日期规则分表使用，
    ...
    /// <summary>
    /// 根据2个固定字段值和1个字段范围值，手动指定<typeparamref name="TMember"/>表分表名执行查询，字段值的顺序与配置的字段顺序保持一致，通常是日期规则分表使用，
    /// .UseTableByRange(1, DateTime.Now.AddDays(-7), DateTime.Now)//商户+时间分表，商户1，最近一周的订单
    /// </summary>
    /// <param name="field1Value">字段1值</param>
