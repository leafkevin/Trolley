...
public interface IQuery<T>
{
    ...
    /// <summary>
    /// 执行SQL查询，返回 T 实体所有字段的记录，记录不存在时返回 Count 为 0 的列表
    /// </summary>
    ...
    /// <summary>
    /// 按照指定的分页设置执行SQL查询，返回 T 实体所有字段的指定条数IPagedList&lt;T&gt;列表，记录不存在时返回 Count 为 0 的IPagedList&lt;T&gt;列表
    /// </summary>
    ...
    /// <summary>
    /// 执行SQL查询，返回 T 实体所有字段的记录并转化为Dictionary&lt;TKey, TValue&gt;字典，记录不存在时返回 Count 为 0 的Dictionary&lt;TKey, TValue&gt;字典
    /// </summary>
