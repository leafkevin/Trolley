...
public interface IUpdate<TEntity>
{
    ...
    #region Sharding
    /// <summary>
    /// 使用固定表名确定TEntity表一个或多个分表名执行更新操作，完整的表名，如：.UseTable("sys_order_202001")，.UseTable("sys_order_202001", "sys_order_202002")，按月分表
    /// </summary>
    /// <param name="tableNames">多个表名，完整的表名，如：sys_order_202001，按月分表</param>
    /// <returns>返回更新对象</returns>
    IUpdate<TEntity> UseTable(params string[] tableNames);
    /// <summary>
    /// 使用表名断言确定TEntity表一个或多个分表执行更新操作，完整的表名，如：.UseTable(f =&gt; f.Contains("202001"))，按月分表
    /// </summary>
    ...
    /// <summary>
    /// 根据字段值确定TEntity表分表名执行更新操作，最多支持2个字段，字段值的顺序与配置的字段顺序保持一致
    /// </summary>
    ...
    /// <summary>
    /// 根据单个字段值范围确定TEntity表分表名执行更新操作，通常是日期规则分表使用，如：repository.From&lt;Order&gt;().UseTableByRange(DateTime.Parse("2020-01-01"), DateTime.Now)
    /// </summary>
    ...
    /// <summary>
    /// 根据1个固定字段值和1个字段值范围确定TEntity表分表名执行更新操作，字段值的顺序与配置的字段顺序保持一致，通常是日期规则分表使用，
