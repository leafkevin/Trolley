...
public class SqlServerContinuedUpdate<TEntity> : ContinuedUpdate<TEntity>, ISqlServerContinuedUpdate<TEntity>
{
    ...
    #region Set
    public new ISqlServerContinuedUpdate<TEntity> Set<TUpdateObj>(TUpdateObj updateObj)
        => this.Set(true, updateObj);
    public new ISqlServerContinuedUpdate<TEntity> Set<TUpdateObj>(bool condition, TUpdateObj updateObj)
    ...
    #region IgnoreFields
    public new ISqlServerContinuedUpdate<TEntity> IgnoreFields(params string[] fieldNames)
        => base.IgnoreFields(fieldNames) as ISqlServerContinuedUpdate<TEntity>;
    public new ISqlServerContinuedUpdate<TEntity> IgnoreFields<TFields>(Expression<Func<TEntity, TFields>> fieldsSelector)
        => base.IgnoreFields(fieldsSelector) as ISqlServerContinuedUpdate<TEntity>;
    #endregion

    #region OnlyFields
    public new ISqlServerContinuedUpdate<TEntity> OnlyFields(params string[] fieldNames)
        => base.OnlyFields(fieldNames) as ISqlServerContinuedUpdate<TEntity>;
    public new ISqlServerContinuedUpdate<TEntity> OnlyFields<TFields>(Expression<Func<TEntity, TFields>> fieldsSelector)
    ...
    public new ISqlServerContinuedUpdate<TEntity> And(Expression<Func<TEntity, bool>> predicate)
        => this.And(true, predicate);
    public new ISqlServerContinuedUpdate<TEntity> And(bool condition, Expression<Func<TEntity, bool>> ifPredicate, Expression<Func<TEntity, bool>> elsePredicate = null)
    ...
    public override int Execute()
    {
        ...
        switch (this.Visitor.ActionMode)
        {
            ...
            case ActionMode.Bulk:
                {
                    var builder = new StringBuilder();
                    (var updateObjs, var bulkCount, var tableName, var fixedParameterSetter, var firstSqlSetter, var sqlSetter, _) = this.Visitor.BuildSetBulk(command);
...
public class SqlServerBulkContinuedUpdate<TEntity> : BulkContinuedUpdate<TEntity>, ISqlServerBulkContinuedUpdate<TEntity>
{
    ...
    #region Constructor
    public SqlServerBulkContinuedUpdate(DbContext dbContext, IUpdateVisitor visitor) : base(dbContext, visitor)
    {
        this.DialectVisitor = this.Visitor as SqlServerUpdateVisitor;
    }
    #endregion
    ...
    #region Set
    public new ISqlServerBulkContinuedUpdate<TEntity> Set<TUpdateObj>(TUpdateObj updateObj)
        => this.Set(true, updateObj);
    public new ISqlServerBulkContinuedUpdate<TEntity> Set<TUpdateObj>(bool condition, TUpdateObj updateObj)
        => base.Set(condition, updateObj) as ISqlServerBulkContinuedUpdate<TEntity>;
    public new ISqlServerBulkContinuedUpdate<TEntity> Set<TField>(Expression<Func<TEntity, TField>> fieldSelector, TField fieldValue)
        => this.Set(true, fieldSelector, fieldValue);
    public new ISqlServerBulkContinuedUpdate<TEntity> Set<TField>(bool condition, Expression<Func<TEntity, TField>> fieldSelector, TField fieldValue)
        => base.Set(condition, fieldSelector, fieldValue) as ISqlServerBulkContinuedUpdate<TEntity>;
    public new ISqlServerBulkContinuedUpdate<TEntity> Set<TFields>(Expression<Func<TEntity, TFields>> fieldsAssignment)
        => this.Set(true, fieldsAssignment);
    public new ISqlServerBulkContinuedUpdate<TEntity> Set<TFields>(bool condition, Expression<Func<TEntity, TFields>> fieldsAssignment)
        => base.Set(condition, fieldsAssignment) as ISqlServerBulkContinuedUpdate<TEntity>;
    #endregion

    #region IgnoreFields
    public new ISqlServerBulkContinuedUpdate<TEntity> IgnoreFields(params string[] fieldNames)
        => base.IgnoreFields(fieldNames) as ISqlServerBulkContinuedUpdate<TEntity>;
    public new ISqlServerBulkContinuedUpdate<TEntity> IgnoreFields<TFields>(Expression<Func<TEntity, TFields>> fieldsSelector)
        => base.IgnoreFields(fieldsSelector) as ISqlServerBulkContinuedUpdate<TEntity>;
    #endregion

    #region OnlyFields
    public new ISqlServerBulkContinuedUpdate<TEntity> OnlyFields(params string[] fieldNames)
        => base.OnlyFields(fieldNames) as ISqlServerBulkContinuedUpdate<TEntity>;
    public new ISqlServerBulkContinuedUpdate<TEntity> OnlyFields<TFields>(Expression<Func<TEntity, TFields>> fieldsSelector)
        => base.OnlyFields(fieldsSelector) as ISqlServerBulkContinuedUpdate<TEntity>;
    #endregion
    ...
    public new ISqlServerBulkContinuedUpdate<TEntity> Where(bool condition, Expression<Func<TEntity, bool>> ifPredicate, Expression<Func<TEntity, bool>> elsePredicate = null)
        => base.Where(condition, ifPredicate, elsePredicate) as ISqlServerBulkContinuedUpdate<TEntity>;
    public new ISqlServerBulkContinuedUpdate<TEntity> And(Expression<Func<TEntity, bool>> predicate)
        => this.And(true, predicate);
            ...
            case ActionMode.BulkCopy:
                {
                    ...
                    //添加临时表
                    var builder = new StringBuilder();
                    builder.AppendLine($"CREATE TABLE {this.OrmProvider.GetTableName(tableName)}(");
                    var pkColumns = new List<string>();
                    foreach ((var refMemberMapper, _) in memberMappers)
                    {
                        var fieldName = this.OrmProvider.GetFieldName(refMemberMapper.FieldName);
