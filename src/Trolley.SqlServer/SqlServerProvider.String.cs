        ...
        switch (memberInfo.Name)
        {
            case "Length":
                formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, (visitor, target) =>
                {
                    ...
                    return targetSegment.Change($"LEN({targetSegment.Body})", false, true);
                });
                result = true;
                break;
        }
                ...
                case "Trim":
                    ...
                    if (parameterInfos.Length == 1 && parameterInfos[0].ParameterType == typeof(char))
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                        {
                            ...
                            var targetArgument = visitor.GetQuotedValue(targetSegment);
                            var rightArgument = visitor.GetQuotedValue(rightSegment);
                            return targetSegment.Merge(rightSegment, $"TRIM({rightArgument} FROM {targetArgument})", false, true);
                        });
                        result = true;
                    }
                    if (parameterInfos.Length == 1 && parameterInfos[0].ParameterType == typeof(char[]))
                ...
                case "TrimStart":
                    ...
                    if (parameterInfos.Length == 1 && parameterInfos[0].ParameterType == typeof(char))
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                        {
                            ...
                            return targetSegment.Merge(rightSegment, $"LTRIM({targetArgument},{rightArgument})", false, true);
                        });
                        result = true;
                    }
                    if (parameterInfos.Length == 1 && parameterInfos[0].ParameterType == typeof(char[]))
                ...
                case "TrimEnd":
                    ...
                    if (parameterInfos.Length == 1 && parameterInfos[0].ParameterType == typeof(char))
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                        {
                            ...
                            var rightArgument = visitor.GetQuotedValue(rightSegment);
                            //sql server 2022支持，其他版本不支持
                            return targetSegment.Merge(rightSegment, $"RTRIM({targetArgument},{rightArgument})", false, true);
                        });
                        result = true;
                    }
                    if (parameterInfos.Length == 1 && parameterInfos[0].ParameterType == typeof(char[]))
                ...
                case "Substring":
                    if (parameterInfos.Length > 1)
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                        {
                            ...
                            return targetSegment.Merge(indexSegment, lengthSegment, $"SUBSTRING({targetArgument},{indexArgument},{lengthArgument})", false, true);
                        });
                        result = true;
                    }
                    else
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                        {
                            ...
                            indexSegment.Value = visitor.GetQuotedValue<int>(indexSegment) + 1;
                            var indexArgument = visitor.GetQuotedValue(indexSegment);
                            return targetSegment.Merge(indexSegment, $"SUBSTRING({targetArgument},{indexArgument})", false, true);
                        });
                        result = true;
                    }
                    break;
