...
public partial class SqlServerProvider : BaseOrmProvider
{
    ...
    static SqlServerProvider()
    {
        ...
        defaultDbTypes[typeof(ushort)] = SqlDbType.Int;
        defaultDbTypes[typeof(int)] = SqlDbType.Int;
        defaultDbTypes[typeof(uint)] = SqlDbType.BigInt;
        defaultDbTypes[typeof(long)] = SqlDbType.BigInt;
        defaultDbTypes[typeof(ulong)] = SqlDbType.Decimal;
            ...
            case "Inserted":
                {
                    ...
                    if (genericArgumentTypes.Length == 1 && methodInfo.DeclaringType == typeof(ISqlServerOutput<>).MakeGenericType(genericArgumentTypes[0]))
                    {
                        ...
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, key =>  (visitor, methodCallExpr, deferredOperations) =>
                        {
                            ...
                            myVisitor.OutputTableAlias = "INSERTED";
            ...
            case "Deleted":
                {
                    ...
                    if (genericArgumentTypes.Length == 1 && methodInfo.DeclaringType == typeof(ISqlServerOutput<>).MakeGenericType(genericArgumentTypes[0]))
                    {
                        ...
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, key =>  (visitor, methodCallExpr, deferredOperations) =>
                        {
                            ...
                            myVisitor.OutputTableAlias = "DELETED";
                            switch (lambdaExpr.Body.NodeType)
                            {
                                case ExpressionType.New:
                                    return myVisitor.VisitNew(new SqlSegment { Expression = lambdaExpr });
