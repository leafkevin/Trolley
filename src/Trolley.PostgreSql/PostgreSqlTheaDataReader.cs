...
class PostgreSqlTheaDataReader : ITheaDataReader
{
    ...
    {
        var schema= await  this.reader.GetColumnSchemaAsync(cancellationToken);
        return schema.Cast<DbColumn>().ToList().AsReadOnly();
    }
#endif
...
#else
        => throw new NotSupportedException(".netstandard2.0以下，接口暂时不支持");
#endif
