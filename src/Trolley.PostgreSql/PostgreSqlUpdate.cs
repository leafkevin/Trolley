...
public class PostgreSqlUpdate<TEntity> : Update<TEntity>, IPostgreSqlUpdate<TEntity>
{
    ...
    #region Sharding
    public new IPostgreSqlUpdate<TEntity> UseTable(params string[] tableNames)
    ...
    public new IPostgreSqlUpdate<TEntity> UseTable<TUpdateObj>(Func<string, TUpdateObj, string> tableNameGetter)
        => base.UseTable(tableNameGetter) as IPostgreSqlUpdate<TEntity>;
    ...
    public new IPostgreSqlUpdate<TEntity> UseTableByRange(object field1Value, object beginField2Value, object endField2Value)
        => base.UseTableByRange(field1Value, beginField2Value, endField2Value) as IPostgreSqlUpdate<TEntity>;
    public new IPostgreSqlUpdate<TEntity> UseTableByRange(object field1Value, object field2Value, object beginField3Value, object endField3Value)
        => base.UseTableByRange(field1Value, field2Value, beginField3Value, endField3Value) as IPostgreSqlUpdate<TEntity>;
    #endregion

    #region Set
    public new IPostgreSqlContinuedUpdate<TEntity> Set<TFields>(TFields setObj)
        => this.Set(true, setObj);
    public new IPostgreSqlContinuedUpdate<TEntity> Set<TFields>(bool condition, TFields setObj)
