            ...
            switch (memberInfo.Name)
            {
                ...
                case "Millisecond":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, (visitor, target) =>
                    {
                        ...
                        var targetArgument = visitor.GetQuotedValue(targetSegment);
                        return targetSegment.Change($"EXTRACT(MILLISECONDS FROM {targetArgument})::INT8-FLOOR(EXTRACT(SECOND FROM {targetArgument}))::INT8*1000", false, false, true);
                    });
                    result = true;
                    break;
                case "Minute":
                ...
                case "Ticks":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, (visitor, target) =>
                    {
                        ...
                        return targetSegment.Change($"EXTRACT(EPOCH FROM {targetSegment})::INT8*10000000+621355968000000000", false, false, true);
                    });
                    result = true;
                    break;
                case "TimeOfDay":
                ...
                case "TryParseExact":
                    formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                    {
                        ...
                        if (formatSegment.IsConstant)
                        {
                            ...
                            else if (!formatArgument.Contains("MM") && formatArgument.Contains("M"))
