            ...
            switch (memberInfo.Name)
            {
                case "Ticks":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, target) =>
                    {
                        ...
                        return targetSegment.Change($"EXTRACT(EPOCH FROM {targetSegment.Value})*10000000", SqlType.MethodCall);
                    });
                    result = true;
                    break;
                case "Hour":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, target) =>
                    {
                        ...
                        return targetSegment.Change($"EXTRACT(HOUR FROM {targetSegment.Value})", SqlType.MethodCall);
                    });
                    result = true;
                    break;
                case "Millisecond":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, target) =>
                    {
                        ...
                        return targetSegment.Change($"EXTRACT(SECOND FROM {targetSegment.Value})*10000000");
                    });
                    result = true;
                    break;
                case "Minute":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, target) =>
                    {
                        ...
                        return targetSegment.Change($"EXTRACT(MINUTE FROM {targetSegment.Value})", SqlType.MethodCall);
                    });
                    result = true;
                    break;
                case "Second":
                    formatter = memberAccessSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, target) =>
                    {
                        ...
                        return targetSegment.Change($"FLOOR(EXTRACT(SECOND FROM {targetSegment.Value}))", SqlType.MethodCall);
                    });
                    result = true;
                    break;
            }
            ...
            switch (methodInfo.Name)
            {
                case "FromTimeSpan":
                    formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, methodCallExpr, deferredOperations) =>
                    {
                        ...
                        return valueSegment.Change($"{this.CastTo(typeof(TimeOnly), valueSegment.Value)}", SqlType.MethodCall);
                    });
                    result = true;
                    break;
                case "FromDateTime":
                ...
                case "TryParse":
                    if (parameterInfos.Length >= 1 && parameterInfos[0].ParameterType == typeof(string))
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, key => (visitor, methodCallExpr, deferredOperations) =>
                        {
                            ...
                            return valueSegment.Change($"{valueSegment.Value}::TIME");
                        });
                        result = true;
                    }
                    break;
