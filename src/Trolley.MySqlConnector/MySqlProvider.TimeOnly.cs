            ...
            switch (methodInfo.Name)
            {
                case "FromTimeSpan":
                    formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                    {
                        ...
                        return valueSegment.Change($"TIME({valueSegment.Body})", false, true);
                    });
                    result = true;
                    break;
                case "FromDateTime":
                ...
                case "TryParseExact":
                    if (parameterInfos.Length >= 2 && parameterInfos[0].ParameterType == typeof(string) && parameterInfos[1].ParameterType == typeof(string))
                    {
                        formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                        {
                            ...
                            return valueSegment.Merge(formatSegment, $"TIME({valueArgument})", false, true);
                        });
                        result = true;
                    }
                    break;
            ...
            switch (methodInfo.Name)
            {
                ...
                case "AddHours":
                    formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                    {
                        ...
                        var targetArgument = visitor.GetQuotedValue(targetSegment);
                        var rightArgument = visitor.GetQuotedValue(rightSegment);
                        return targetSegment.Merge(rightSegment, $"TIME_ADD({targetArgument},INTERVAL {rightArgument} HOUR)", false, true);
                    });
                    result = true;
                    break;
                case "AddMinutes":
                    formatter = methodCallSqlFormatterCache.GetOrAdd(cacheKey, (visitor, orgExpr, target, deferExprs, args) =>
                    {
                        ...
                        var targetArgument = visitor.GetQuotedValue(targetSegment);
                        var rightArgument = visitor.GetQuotedValue(rightSegment);
                        return targetSegment.Merge(rightSegment, $"TIME_ADD({targetArgument},INTERVAL {rightArgument} MINUTE)", false, true);
                    });
                    result = true;
                    break;
                case "CompareTo":
