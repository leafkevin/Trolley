...
SELECT b.[MenuId],a.[Name],b.[ParentId],a.[PageId],b.[Url] FROM [sys_menu] a INNER JOIN [menuPageList] b ON a.[Id]=b.[MenuId] AND a.[PageId]>@p1");
        ...
        Assert.True(dbParameters[0].ParameterName == "@MenuId");
...
SELECT a.[MenuId],a.[ParentId],a.[Url] FROM [menuPageList] a WHERE a.[ParentId]<@p2");
        ...
        Assert.True(dbParameters[1].ParameterName == "@MenuId");
    ...
    public async Task FromQuery_Include()
    {
        ...
        var sql = repository.From<Product>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[ProductNo],a.[Name],a.[BrandId],a.[CategoryId],a.[Price],a.[CompanyId],a.[IsEnabled],a.[CreatedAt],a.[CreatedBy],a.[UpdatedAt],a.[UpdatedBy],b.[Id],b.[BrandNo],b.[Name] FROM [sys_product] a LEFT JOIN [sys_brand] b ON a.[BrandId]=b.[Id] WHERE a.[ProductNo] LIKE N'%PN-00%'");
    ...
    public void FromQuery_Ignore_Include()
    {
        ...
        var sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) AS [Date],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) ORDER BY a.[Id],CONVERT(DATE,b.[CreatedAt])");
        var result = repository.From<User>()
            .InnerJoin<Order>((x, y) => x.Id == y.BuyerId)
            //.IncludeMany((a, b) => a.Orders)
            //.ThenIncludeMany(f => f.Details)
    ...
    public void FromQuery_Groupby()
    {
        ...
        var sql1 = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql1 == "SELECT a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) AS [Date],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) ORDER BY a.[Id]");
        var result1 = repository.From<User>()
            .InnerJoin<Order>((x, y) => x.Id == y.BuyerId)
            .GroupBy((a, b) => new { a.Id, a.Name, b.CreatedAt.Date })
            .OrderBy((x, a, b) => new { UserId = a.Id })
    ...
    public void FromQuery_Groupby_Fields()
    {
        ...
        var sql = repository.From<User>()
           ...
           .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id] AS [UserId1],a.[Name] AS [UserName],CONVERT(DATE,b.[CreatedAt]) AS [CreatedDate1],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) ORDER BY a.[Id]");
        var result = repository.From<User>()
            .InnerJoin<Order>((x, y) => x.Id == y.BuyerId)
            .GroupBy((a, b) => new { UserId = a.Id, a.Name, CreatedDate = b.CreatedAt.Date })
            .OrderBy((x, a, b) => new { UserId = a.Id })
    ...
    public void FromQuery_Groupby_OrderBy()
    {
        ...
        var sql = repository.From<User>()
           ...
           .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) AS [Date],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) ORDER BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt])");
        var result = repository.From<User>()
          .InnerJoin<Order>((x, y) => x.Id == y.BuyerId)
          .GroupBy((a, b) => new { a.Id, a.Name, b.CreatedAt.Date })
          .OrderBy((x, a, b) => x.Grouping)
    ...
    public async Task FromQuery_Groupby_OrderBy_Fields()
    {
        ...
        var sql = repository.From<User>()
           ...
           .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id] AS [UserId],a.[Name],CONVERT(DATE,b.[CreatedAt]) AS [CreatedDate],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) ORDER BY a.[Id],a.[Name] DESC,CONVERT(DATE,b.[CreatedAt])");
    ...
    public void FromQuery_Groupby_Having_OrderBy_Fields()
    {
        ...
        var sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) AS [Date],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] WHERE EXISTS(SELECT * FROM [sys_order_detail] f WHERE b.[Id]=f.[OrderId] AND f.[ProductId]=2) GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) HAVING SUM(b.[TotalAmount])>300 ORDER BY a.[Id],a.[Name] DESC,CONVERT(DATE,b.[CreatedAt])");
        var result = repository.From<User>()
            .InnerJoin<Order>((x, y) => x.Id == y.BuyerId)
            .Where((a, b) => Sql.Exists<OrderDetail>(f => b.Id == f.OrderId && f.ProductId == 2))
            .GroupBy((a, b) => new { a.Id, a.Name, b.CreatedAt.Date })
    ...
    public void Where_Exists()
    {
        ...
        var sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[TenantId],a.[Name],a.[Gender],a.[Age],a.[CompanyId],a.[GuidField],a.[SomeTimes],a.[SourceType],a.[IsEnabled],a.[CreatedAt],a.[CreatedBy],a.[UpdatedAt],a.[UpdatedBy] FROM [sys_user] a WHERE EXISTS(SELECT * FROM [sys_company] t WHERE t.[Name] LIKE N'%谷歌%' AND a.[CompanyId]=t.[Id])");
        var result = repository.From<User>()
            .Where(f => repository.Exists<Company>(t => t.Name.Contains("谷歌") && f.CompanyId == t.Id))
            .ToList();
        Assert.NotNull(result);
        ...
        sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[TenantId],a.[Name],a.[Gender],a.[Age],a.[CompanyId],a.[GuidField],a.[SomeTimes],a.[SourceType],a.[IsEnabled],a.[CreatedAt],a.[CreatedBy],a.[UpdatedAt],a.[UpdatedBy] FROM [sys_user] a WHERE EXISTS(SELECT * FROM [sys_company] b WHERE b.[Name] LIKE N'%谷歌%' AND a.[CompanyId]=b.[Id])");
        ...
        var sql1 = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql1 == "SELECT a.[Gender],a.[CompanyId],COUNT(DISTINCT a.[Id]) AS [UserTotal] FROM [sys_user] a WHERE EXISTS(SELECT b.[Id] FROM [sys_order] b,[sys_order_detail] c WHERE b.[Id]=c.[OrderId] AND a.[Id]=b.[BuyerId] GROUP BY b.[Id] HAVING COUNT(DISTINCT c.[ProductId])>1) GROUP BY a.[Gender],a.[CompanyId]");
        var result1 = repository.From<User>()
            .Where(f => Sql.Exists(t => t
                .From<Order, OrderDetail>('b')
                .Where((x, y) => x.Id == y.OrderId && f.Id == x.BuyerId)
...
SELECT a.[Id],a.[Name],b.[Name] AS [CompanyName] FROM [sys_user] a INNER JOIN [sys_company] b ON a.[CompanyId]=b.[Id] WHERE EXISTS(SELECT * FROM [myOrders] f WHERE f.[BuyerId]=a.[Id])");
    ...
    public void FromQuery_In_Exists()
    {
        ...
        sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) AS [Date],COUNT(b.[Id]) AS [OrderCount],SUM(b.[TotalAmount]) AS [TotalAmount] FROM [sys_user] a INNER JOIN [sys_order] b ON a.[Id]=b.[BuyerId] WHERE a.[Id] IN (1,2,3) AND EXISTS(SELECT * FROM [sys_order_detail] f WHERE b.[Id]=f.[OrderId] AND f.[ProductId]=2) GROUP BY a.[Id],a.[Name],CONVERT(DATE,b.[CreatedAt]) HAVING SUM(b.[TotalAmount])>300 ORDER BY a.[Id],CONVERT(DATE,b.[CreatedAt])");
        result = repository.From<User>()
            .InnerJoin<Order>((x, y) => x.Id == y.BuyerId)
            .Where((a, b) => Sql.In(a.Id, new int[] { 1, 2, 3 }) && Sql.Exists<OrderDetail>(f => b.Id == f.OrderId && f.ProductId == 2))
            .GroupBy((a, b) => new { a.Id, a.Name, b.CreatedAt.Date })
    ...
    public async Task Query_Where_IsNull()
    {
        ...
        var sql1 = repository.From<Order>()
           ...
           .ToSql(out _);
        Assert.True(sql1 == "SELECT a.[Id],a.[OrderNo],ISNULL(a.[ProductCount],0) AS [ProductCount],ISNULL(a.[BuyerId],0) AS [BuyerId],ISNULL(a.[TotalAmount],0) AS [TotalAmount] FROM [sys_order] a WHERE ISNULL(a.[ProductCount],0)>0 OR ISNULL(a.[BuyerId],0)>=0");
...
SELECT a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]<>@p1");
    ...
    public async Task Query_Union_Take()
    {
        ...
        var sql = repository
            ...
            .ToSql(out var dbParameters);
        Assert.True(sql == @"SELECT a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId],b.[Name] AS [BuyerName] FROM (SELECT * FROM (SELECT TOP 1 b.[Id],b.[OrderNo],b.[SellerId],b.[BuyerId] FROM [sys_order] b WHERE b.[Id]=@p0 ORDER BY b.[Id]) a UNION ALL
SELECT a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]<>@p1) a INNER JOIN [sys_user] b ON a.[BuyerId]=b.[Id]");
        ...
        Assert.True(sql1 == @"SELECT b.[Id],b.[OrderNo],b.[SellerId],a.[Name] AS [SellerName],b.[BuyerId],c.[Name] AS [BuyerName] FROM [sys_user] a INNER JOIN (SELECT * FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a INNER JOIN [sys_user] b ON a.[SellerId]=b.[Id] WHERE a.[Id]=@p0 ORDER BY a.[Id]) a UNION ALL
    ...
    public async Task FromQuery_Union_Limit()
    {
        ...
        var sql = repository.From<Order>()
            ...
            .UnionAll(f => f.From<Order>()
                ...
                .Take(1))
            .ToSql(out var dbParameters);
        Assert.True(sql == @"SELECT * FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]=@p0 ORDER BY a.[Id]) a UNION ALL
SELECT * FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]<>@p1) a");
    ...
    public void FromQuery_Union_SubQuery_Limit()
    {
        ...
        var sql = repository.From(f => f.From<Order>()
            ...
            .UnionAll(f => f.From<Order>()
                ...
                .Take(1))
            .ToSql(out _);
        Assert.True(sql == @"SELECT a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]<>N'3' ORDER BY a.[Id]) a UNION ALL
SELECT * FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]=N'3') a");
    ...
    public async Task FromQuery_Union_SubQuery_OrderBy()
    {
        ...
        var sql = repository.From<Order>()
            ...
            .ToSql(out var dbParameters);
        Assert.True(sql == @"SELECT * FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]=@p0 ORDER BY a.[Id]) a UNION ALL
SELECT * FROM (SELECT TOP 1 a.[Id],a.[OrderNo],a.[SellerId],a.[BuyerId] FROM [sys_order] a WHERE a.[Id]<>@p1 ORDER BY a.[Id] DESC) a");
        Assert.True(dbParameters.Count == 2);
        Assert.True((string)dbParameters[0].Value == id1);
        Assert.True((string)dbParameters[1].Value == id2);
...
SELECT * FROM (SELECT TOP 1 a.[BuyerId] AS [Id],a.[OrderNo] AS [Name],a.[SellerId] AS [ParentId],CAST(a.[BuyerId] AS NVARCHAR(MAX)) AS [Url] FROM [sys_order] a WHERE a.[Id]=N'2' ORDER BY a.[Id] DESC) a");
...
SELECT a.[Id],a.[Name],a.[ParentId],b.[Url] FROM [MenuList] a INNER JOIN [sys_page] b ON a.[Id]=b.[Id] WHERE b.[Id]>=@p1");
    ...
    public async Task Query_WithNextCte()
    {
        ...
        Assert.True(sql == @"WITH [myCteTable1]([Id],[Name],[ParentId]) AS 
(
...
SELECT b.[Id],a.[Url] FROM [sys_page] a INNER JOIN [MenuList] b ON a.[Id]=b.[Id] WHERE a.[Id]>@p2) b ON a.[Id]=b.[Id]");
...
SELECT a.[Id],a.[Name],a.[ParentId],b.[Url] FROM [MenuList] a INNER JOIN [MenuPageList] b ON a.[Id]=b.[Id]");
    ...
    public async Task Query_WithTable()
    {
        ...
        var sql1 = repository
            ...
            .Select((a, b) => new { a.Id, a.Name, a.ParentId, b.Url })
            .ToSql(out var dbParameters);
        Assert.True(sql1 == @"WITH [myCteTable1]([Id],[Name],[ParentId]) AS 
(
SELECT a.[Id],a.[Name],a.[ParentId] FROM [sys_menu] a WHERE a.[Id]=@p0 UNION ALL
SELECT a.[Id],a.[Name],a.[ParentId] FROM [sys_menu] a INNER JOIN [myCteTable1] b ON a.[ParentId]=b.[Id]
),
...
SELECT a.[Id],a.[Name],a.[ParentId],b.[Url] FROM [myCteTable1] a INNER JOIN [myCteTable2] b ON a.[Id]=b.[Id]");
        ...
        var sql2 = repository.From<Order, OrderDetail>()
            ...
            .ToSql(out _);
        Assert.True(sql2 == "SELECT a.[Id],a.[TenantId],a.[OrderNo],a.[ProductCount],a.[TotalAmount],a.[BuyerId],a.[BuyerSource],a.[SellerId],a.[Products],a.[Disputes],a.[IsEnabled],a.[CreatedAt],a.[CreatedBy],a.[UpdatedAt],a.[UpdatedBy],c.[Id],c.[TenantId],c.[Name],c.[Gender],c.[Age],c.[CompanyId],c.[GuidField],c.[SomeTimes],c.[SourceType],c.[IsEnabled],c.[CreatedAt],c.[CreatedBy],c.[UpdatedAt],c.[UpdatedBy],a.[BuyerId],b.[Id] AS [DetailId],b.[Price],b.[Quantity],b.[Amount] FROM [sys_order] a INNER JOIN [sys_order_detail] b ON a.[Id]=b.[OrderId] LEFT JOIN [sys_user] c ON a.[BuyerId]=c.[Id] WHERE a.[Id]=b.[OrderId]");
    ...
    public void SelectFlattenTo()
    {
        ...
        {
            Assert.NotNull(result1);
            Assert.True(!string.IsNullOrEmpty(result1.OrderId));
