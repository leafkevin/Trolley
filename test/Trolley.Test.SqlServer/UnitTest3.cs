...
public class UnitTest3 : UnitTestBase
{
    ...
    public async void Update_Set_FromQuery_One()
    {
        ...
        var sql = repository.Update<Order>()
            ...
            .Where(a => a.Id == "1")
            .ToSql(out var dbParameters);
        Assert.True(sql == "UPDATE [sys_order] SET [TotalAmount]=(SELECT SUM(b.[Amount]) FROM [sys_order_detail] b WHERE b.[OrderId]=[sys_order].[Id]),[OrderNo]=@OrderNo,[BuyerId]=NULL WHERE [Id]=N'1'");
        Assert.True(dbParameters.Count == 1);
    ...
    public void Update_Set_FromQuery_One_Enum()
    {
        ...
        var sql = repository.Update<Company>()
            ...
            .ToSql(out _);
        Assert.True(sql == "UPDATE [sys_company] SET [Nature]=(SELECT b.[Nature] FROM [sys_company] b WHERE b.[Id]=1) WHERE [Nature]<>N'Internet'");
 		repository.BeginTransaction();
    ...
    public async void Update_InnerJoin_Fields()
    {
        ...
        var sql = repository.Update<Order>()
            ...
            .Where((a, b) => a.Id == "1")
            .ToSql(out _);
        Assert.True(sql == "UPDATE a SET a.[TotalAmount]=(SELECT SUM(c.[Amount]) FROM [sys_order_detail] c WHERE c.[OrderId]=a.[Id]),a.[OrderNo]=a.[OrderNo]+' - '+CAST(b.[Id] AS NVARCHAR(MAX)),a.[BuyerId]=NULL FROM [sys_order] a INNER JOIN [sys_user] b ON a.[BuyerId]=b.[Id] WHERE a.[Id]=N'1'");
        ...
        var sql1 = repository.Update<Order>()
            ...
            .Where((x, y) => x.Id == "2")
            .ToSql(out _);
        Assert.True(sql1 == "UPDATE a SET a.[TotalAmount]=(SELECT SUM(c.[Amount]) FROM [sys_order_detail] c WHERE c.[OrderId]=a.[Id]),a.[OrderNo]=a.[OrderNo]+' - '+CAST(b.[Id] AS NVARCHAR(MAX)),a.[BuyerId]=NULL FROM [sys_order] a INNER JOIN [sys_user] b ON a.[BuyerId]=b.[Id] WHERE a.[Id]=N'2'");
