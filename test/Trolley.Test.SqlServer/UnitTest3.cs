...
public class UnitTest3 : UnitTestBase
{
    ...
    public void Update_Set_MethodCall()
    {
        ...
        var sql = repository.Update<Order>()
            ...
            .ToSql(out var dbParameters);
        Assert.True(sql == "UPDATE [sys_order] SET [TotalAmount]=@p0,[Products]=@p1,[Disputes]=@p2,[UpdatedAt]=GETDATE() WHERE [Id]=@kId");
    ...
    public void Update_Set_FromQuery_Multi()
    {
        ...
        var sql = repository.Update<Order>()
          ...
          .ToSql(out var dbParameters);
        Assert.True(sql == "UPDATE a SET a.[TotalAmount]=@p0,a.[OrderNo]=a.[OrderNo]+'-111',a.[BuyerSource]=b.[SourceType],a.[Products]=@Products FROM [sys_order] a INNER JOIN [sys_user] b ON a.[BuyerId]=b.[Id] WHERE a.[BuyerId]=1");
        ...
        sql = repository.Update<Order>()
            ...
            .Where(a => a.BuyerId == 1)
            .ToSql(out _);
        Assert.True(sql == "UPDATE [sys_order] SET [TotalAmount]=(SELECT SUM(b.[Amount]) FROM [sys_order_detail] b WHERE b.[OrderId]=[sys_order].[Id]),[OrderNo]=[OrderNo]+'_111',[BuyerId]=NULL WHERE [BuyerId]=1");
    ...
    public async void Update_Set_FromQuery_One()
    {
        ...
        var sql = repository.Update<Order>()
            ...
            .Where(a => a.Id == "1")
            .ToSql(out var dbParameters);
        Assert.True(sql == "UPDATE [sys_order] SET [TotalAmount]=(SELECT SUM(b.[Amount]) FROM [sys_order_detail] b WHERE b.[OrderId]=[sys_order].[Id]),[OrderNo]=@OrderNo,[BuyerId]=NULL WHERE [Id]='1'");
        Assert.True(dbParameters.Count == 1);
    ...
    public async void Update_InnerJoin_Multi()
    {
        ...
        var sql = repository.Update<Order>()
            ...
            .Where((a, b) => a.BuyerId == 1)
            .ToSql(out _);
        Assert.True(sql == "UPDATE a SET a.[TotalAmount]=b.[Amount],a.[OrderNo]=a.[OrderNo]+'_111',a.[BuyerId]=NULL FROM [sys_order] a INNER JOIN [sys_order_detail] b ON a.[Id]=b.[OrderId] WHERE a.[BuyerId]=1");
    ...
    public async void Update_InnerJoin_Fields()
    {
        ...
        var sql = repository.Update<Order>()
            ...
            .Where((a, b) => a.Id == "1")
            .ToSql(out _);
        Assert.True(sql == "UPDATE a SET a.[TotalAmount]=(SELECT SUM(c.[Amount]) FROM [sys_order_detail] c WHERE c.[OrderId]=a.[Id]),a.[OrderNo]=a.[OrderNo]+' - '+CAST(b.[Id] AS NVARCHAR(MAX)),a.[BuyerId]=NULL FROM [sys_order] a INNER JOIN [sys_user] b ON a.[BuyerId]=b.[Id] WHERE a.[Id]='1'");
        ...
        var sql1 = repository.Update<Order>()
            ...
            .Where((x, y) => x.Id == "2")
            .ToSql(out _);
        Assert.True(sql1 == "UPDATE a SET a.[TotalAmount]=(SELECT SUM(c.[Amount]) FROM [sys_order_detail] c WHERE c.[OrderId]=a.[Id]),a.[OrderNo]=a.[OrderNo]+' - '+CAST(b.[Id] AS NVARCHAR(MAX)),a.[BuyerId]=NULL FROM [sys_order] a INNER JOIN [sys_user] b ON a.[BuyerId]=b.[Id] WHERE a.[Id]='2'");
