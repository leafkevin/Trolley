...
public class DateTimeUnitTest : UnitTestBase
{
    ...
    public async void MemberAccess()
    {
        ...
        Assert.True(sql == "SELECT GETDATE() AS [Now],'0001-01-01 00:00:00.000' AS [MinValue],'9999-12-31 23:59:59.999' AS [MaxValue],GETUTCDATE() AS [UtcNow],CONVERT(DATE,GETDATE()) AS [Today],'1970-01-01 00:00:00.000' AS [UnixEpoch],'2023-05-06 00:00:00.000' AS [Date],CONVERT(DATE,GETDATE()) AS [CurrentDate],@p0 AS [localDate],(CASE WHEN a.[UpdatedAt]='2023-03-25 00:00:00.000' THEN 1 ELSE 0 END) AS [IsEquals],(CASE WHEN a.[UpdatedAt]=@p1 THEN 1 ELSE 0 END) AS [IsEquals1] FROM [sys_user] a WHERE a.[Id]=1");
    ...
    public async void AddSubtract()
    {
        ...
        var sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT DATEADD(DAY,365,a.[CreatedAt]) AS [Add],DATEADD(DAY,30,a.[CreatedAt]) AS [AddDays],DATEADD(MILLISECOND,300,a.[CreatedAt]) AS [AddMilliseconds],(DATEADD(DAY,-365,a.[CreatedAt])) AS [Subtract1],(DATEADD(DAY,-365,GETDATE())) AS [Subtract2],(CASE WHEN DATEDIFF(SECOND,a.[CreatedAt],a.[UpdatedAt])>0 THEN CAST(DATEDIFF(DAY,a.[CreatedAt],a.[UpdatedAt]) AS VARCHAR)+'.'+CAST(CONVERT(TIME,DATEADD(SECOND,DATEDIFF(SECOND,a.[CreatedAt],a.[UpdatedAt])%86400,'00:00:00')) AS VARCHAR) ELSE CAST(DATEDIFF(DAY,a.[CreatedAt],a.[UpdatedAt])+1 AS VARCHAR)+'.'+CAST(CONVERT(TIME,DATEADD(SECOND,86400-DATEDIFF(SECOND,a.[CreatedAt],a.[UpdatedAt])%86400,'00:00:00')) AS VARCHAR) END) AS [Subtract3],DAY(EOMONTH(CAST(DATEPART(YEAR,GETDATE()) AS NVARCHAR(4))+'-'+CAST(DATEPART(MONTH,GETDATE()) AS NVARCHAR(2))+'-01')) AS [DayInMonth],(CASE WHEN DATEPART(YEAR,GETDATE())%4=0 AND DATEPART(YEAR,GETDATE())%100<>0 OR DATEPART(YEAR,GETDATE())%400=0 THEN 1 ELSE 0 END) AS [IsLeapYear1],1 AS [IsLeapYear2],CAST(FORMAT(GETDATE(),'yyyy-MM-dd HH:mm:ss') AS DATETIME) AS [Parse],'2023-05-07 13:08:45.000' AS [ParseExact] FROM [sys_user] a WHERE a.[UpdatedAt]>(DATEADD(MILLISECOND,-1500000,DATEADD(DAY,-365,GETDATE())))");
    ...
    public async void Compare()
    {
        ...
        var sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT (CASE WHEN a.[CreatedAt]='2023-03-03 00:00:00.000' THEN 0 WHEN a.[CreatedAt]>'2023-03-03 00:00:00.000' THEN 1 ELSE -1 END) AS [CompareTo],(DATEADD(DAY,-365,a.[CreatedAt])) AS [OneYearsAgo1],(CASE WHEN DATEDIFF(SECOND,'2023-03-20 00:00:00.000',a.[CreatedAt])>0 THEN CAST(DATEDIFF(DAY,'2023-03-20 00:00:00.000',a.[CreatedAt]) AS VARCHAR)+'.'+CAST(CONVERT(TIME,DATEADD(SECOND,DATEDIFF(SECOND,'2023-03-20 00:00:00.000',a.[CreatedAt])%86400,'00:00:00')) AS VARCHAR) ELSE CAST(DATEDIFF(DAY,'2023-03-20 00:00:00.000',a.[CreatedAt])+1 AS VARCHAR)+'.'+CAST(CONVERT(TIME,DATEADD(SECOND,86400-DATEDIFF(SECOND,'2023-03-20 00:00:00.000',a.[CreatedAt])%86400,'00:00:00')) AS VARCHAR) END) AS [OneYearsAgo2],DAY(EOMONTH(CAST(DATEPART(YEAR,GETDATE()) AS NVARCHAR(4))+'-'+CAST(DATEPART(MONTH,GETDATE()) AS NVARCHAR(2))+'-01')) AS [DayInMonth],(CASE WHEN DATEPART(YEAR,GETDATE())%4=0 AND DATEPART(YEAR,GETDATE())%100<>0 OR DATEPART(YEAR,GETDATE())%400=0 THEN 1 ELSE 0 END) AS [IsLeapYear1],1 AS [IsLeapYear2],CAST(FORMAT(GETDATE(),'yyyy-MM-dd HH:mm:ss') AS DATETIME) AS [Parse],'2023-05-07 13:08:45.000' AS [ParseExact] FROM [sys_user] a WHERE (CASE WHEN a.[UpdatedAt]='2023-03-20 00:00:00.000' THEN 0 WHEN a.[UpdatedAt]>'2023-03-20 00:00:00.000' THEN 1 ELSE -1 END)>0");
    ...
    public async void Operation()
    {
        ...
        var sql = repository.From<User>()
            ...
            .ToSql(out _);
        Assert.True(sql == "SELECT '05:06:07.000000' AS [DateSub],DATEADD(MILLISECOND,18000000,a.[CreatedAt]) AS [AddOp],(DATEADD(MILLISECOND,-36000000,a.[CreatedAt])) AS [SubOp],CAST(DATEADD(SECOND,DATEDIFF(SECOND,'00:00:00',a.[SomeTimes])+DATEDIFF(SECOND,'00:00:00','00:25:00.000000'),'00:00:00') AS TIME) AS [AddOp1],'1.05:45:00.000000' AS [SubOp1],(CASE WHEN DATEDIFF(SECOND,a.[CreatedAt],a.[UpdatedAt])>0 THEN CAST(DATEDIFF(DAY,a.[CreatedAt],a.[UpdatedAt]) AS VARCHAR)+'.'+CAST(CONVERT(TIME,DATEADD(SECOND,DATEDIFF(SECOND,a.[CreatedAt],a.[UpdatedAt])%86400,'00:00:00')) AS VARCHAR) ELSE CAST(DATEDIFF(DAY,a.[CreatedAt],a.[UpdatedAt])+1 AS VARCHAR)+'.'+CAST(CONVERT(TIME,DATEADD(SECOND,86400-DATEDIFF(SECOND,a.[CreatedAt],a.[UpdatedAt])%86400,'00:00:00')) AS VARCHAR) END) AS [SubOp2],'01:15:00.000000' AS [MulOp],'06:00:00.000000' AS [DivOp1],10 AS [DivOp2] FROM [sys_user] a WHERE (CASE WHEN a.[UpdatedAt]='2023-03-20 00:00:00.000' THEN 0 WHEN a.[UpdatedAt]>'2023-03-20 00:00:00.000' THEN 1 ELSE -1 END)>0");
        var result = await repository.From<User>()
            .Where(f => f.Id == 1)
            .Select(f => new
            {
