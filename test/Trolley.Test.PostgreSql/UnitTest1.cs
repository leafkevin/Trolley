...
public class UnitTest1 : UnitTestBase
{
    ...
    public async void Insert_OnConflictDoUpdate()
    {
        ...
        var sql2 = repository.Create<Order>()
             ...
             .OnConflict(x => x
                .Set(f => new { TotalAmount = x.Excluded(f.TotalAmount) })
        ...
        var sql3 = repository.Create<Order>()
             ...
             .OnConflict(x => x
                ...
                .Set(f => f.Products, f => x.Excluded(f.Products)))
            .ToSql(out _);
        Assert.True(sql3 == "INSERT INTO `sys_order` (`Id`,`TenantId`,`OrderNo`,`BuyerId`,`SellerId`,`BuyerSource`,`TotalAmount`,`Products`,`Disputes`,`IsEnabled`,`CreatedAt`,`CreatedBy`,`UpdatedAt`,`UpdatedBy`) VALUES (@Id,@TenantId,@OrderNo,@BuyerId,@SellerId,@BuyerSource,@TotalAmount,@Products,@Disputes,@IsEnabled,@CreatedAt,@CreatedBy,@UpdatedAt,@UpdatedBy) AS newRow ON DUPLICATE KEY UPDATE `TotalAmount`=newRow.`TotalAmount`,`Products`=newRow.`Products`");
        ...
        var count = await repository.Create<Order>()
             ...
             .OnConflict(x => x.Set(f => new
             {
                 TotalAmount = x.Excluded(f.TotalAmount),
                 Products = x.Excluded(f.Products)
             }))
            .ExecuteAsync();
        ...
        count = await repository.Create<Order>()
            ...
            .OnConflict(x => x
                .Set(f => new { TotalAmount = x.Excluded(f.TotalAmount) })
                .Set(true, f => f.Products, f => x.Excluded(f.Products)))
            .ExecuteAsync();
