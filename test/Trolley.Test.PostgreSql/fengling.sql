...
CREATE OR REPLACE FUNCTION GET_USER(IN pId int4)
RETURNS SETOF sys_user AS $$
BEGIN
	RETURN QUERY SELECT * FROM sys_user WHERE "Id" = pId;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE PROCEDURE UPDATE_USER(IN pId int4, OUT pOut varchar(50)) AS $BODY$
BEGIN
	pOut := 'OK';
