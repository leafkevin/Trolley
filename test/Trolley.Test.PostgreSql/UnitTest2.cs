...
SELECT a.""Id"",a.""Name"",a.""ParentId"",b.""Url"" FROM ""myCteTable1"" a INNER JOIN ""myCteTable2"" b ON a.""Id""=b.""Id""", sql1);
    ...
    public async Task FromQuery_StringAgg()
    {
        ...
        var sql = repository.From<Order>()
            ...
            .ToSql(out _);
        Assert.AreEqual("SELECT a.\"ProductCount\",STRING_AGG(CONCAT(CAST(a.\"BuyerId\" AS VARCHAR),':',CAST(a.\"ProductCount\" AS VARCHAR)),',' ORDER BY a.\"BuyerId\",a.\"OrderNo\",a.\"CreatedAt\" DESC) AS \"Buyer_ProductCount\" FROM \"sys_order\" a INNER JOIN \"sys_user\" b ON a.\"BuyerId\"=b.\"Id\" GROUP BY a.\"ProductCount\"", sql);
    ...
    public async Task SelectAndOrExpr()
    {
        ...
        var sql = repository.From<Product, Brand>()
            ...
            .ToSql(out _);
        Assert.AreEqual("SELECT (CASE WHEN a.\"IsEnabled\"=TRUE AND b.\"IsEnabled\"=TRUE OR a.\"CompanyId\" IS NULL THEN TRUE ELSE FALSE END) AS \"IsEnabled\" FROM \"sys_product\" a INNER JOIN \"sys_brand\" b ON a.\"BrandId\"=b.\"Id\"", sql);
