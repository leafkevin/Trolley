...
class Create<T> : ICreate<T>
{
    ...
    public ICreate<T> From<TEntity>(Expression<Func<TEntity, T>> fields, Expression<Func<TEntity, bool>> predicate)
    {
        this.isWithBy = false;
        var entityType = typeof(TEntity);
        this.visitor = new QueryVisitor(this.dbFactory, this.connection.OrmProvider);
        this.visitor.From(entityType);
        this.visitor.Where(predicate);
        this.visitor.Select(null, fields);
        return this;
    }
    public int Execute()
    {
    ...
    public string ToSql()
    {
        ...
        {
            var sql = this.visitor.BuildSql(out _, out var readerFields);
            var builder = new StringBuilder("INSERT INTO (");
            for (int i = 0; i < readerFields.Count; i++)
            {
                var readerField = readerFields[i];
