...
public class MySqlProvider : BaseOrmProvider
{
    ...
    public MySqlProvider()
    {
        ...
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.In), 1, new Type[] { Type.MakeGenericMethodParameter(0), Type.MakeGenericMethodParameter(0).MakeArrayType() }), (target, deferredExprs, arguments) => "COUNT(1)");
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.In), Type.EmptyTypes), (target, deferredExprs, arguments) => "COUNT(1)");
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.In), Type.EmptyTypes), (target, deferredExprs, arguments) => "COUNT(1)");
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.In), Type.EmptyTypes), (target, deferredExprs, arguments) => "COUNT(1)");
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.In), Type.EmptyTypes), (target, deferredExprs, arguments) => "COUNT(1)");
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.In), Type.EmptyTypes), (target, deferredExprs, arguments) => "COUNT(1)");
        //methodCallSqlFormatterCahe.TryAdd(typeof(IWhereSql).GetMethod(nameof(IWhereSql.Exists), Type.EmptyTypes), (target, deferredExprs, arguments) => "COUNT(1)");
    }
        ...
        if (!methodCallSqlFormatterCahe.TryGetValue(methodInfo, out formatter))
        {
            ...
            switch (methodInfo.Name)
            {
                case "In":
                    ...
                    if (methodInfo.DeclaringType == typeof(IWhereSql))
                    {
                        var genericType = methodInfo.GetGenericArguments().First();
                        if (typeof(IQuery<>).MakeGenericType(genericType).IsAssignableFrom(parameterInfos[1].ParameterType))
                            //数组调用
